...
export function buildContentPackPrompt(payload = {}) {
  ...
  return [
    ...
    "Write 2 or 3 distinct content pack variations for the same doll.",
    "Each variation must stay aligned with the same brand voice and the same universe and character context.",
    "Each variation must include:",
    '- "id": a machine-safe identifier such as "v1", "v2", or "v3"',
    '- "label": a short operator-friendly label such as "Tender & premium"',
    '- "short_intro": a short warm intro suitable for a caption opener',
    '- "content_blurb": a short brand-style blurb for the doll',
    '- "promo_hook": one concise feeling-led hook',
    '- "cta": a short gentle call to action',
    "Return valid JSON only with this exact shape:",
    "{",
    '  "variations": [',
    '    { "id": "...", "label": "...", "short_intro": "...", "content_blurb": "...", "promo_hook": "...", "cta": "..." }',
    "  ]",
    "}",
    "Do not include markdown, code fences, or extra keys.",
    "",
    "Field guidance",
    '- Each variation should feel emotionally distinct while staying inside the same brand world.',
