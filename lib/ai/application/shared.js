...
export async function executeAIApplicationAction({
  ...
  try {
    ...
    const forwardedProvider =
      typeof command.metadata?.provider === "string" &&
      command.metadata.provider.trim() &&
      command.metadata.provider.trim() !== "anthropic"
        ? command.metadata.provider.trim()
        : undefined;
    ...
    const normalizedData = {
      ...
      provider:
        ...
        readOptionalString(forwardedProvider),
